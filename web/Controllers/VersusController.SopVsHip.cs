...
namespace Entt.Ers.Controllers
{
    public partial class VersusController
    {
        ...
        public ActionResult SopVsHip(PrefixReportViewModel model)
        {
            var reportViewer = ReportEngine.Create();
            ...
            if (ModelState.IsValid)
            {
                ...
                dataset = model.SelectedBranch == "All" ?
                    m_enttContext.GetSopVsHipReportDataSet(model.ReportDate, day) :
                    m_enttContext.GetSopVsHipBranchReportDataSet(model.ReportDate, day, model.SelectedBranch);
                reportViewer.LocalReport.ReportPath = Request.MapPath(Request.ApplicationPath) + @"Reports\Versus\SopVsHip.rdlc";
        ...
        public ActionResult SopVsHipDetails(string branchCode, double date, string day = "7")
        {
            ...
            var dataset = m_enttContext.GetSopVsHipDetailsReportDataSet(reportDate.Date, int.Parse(day), branchCode);
            reportViewer.LocalReport.ReportPath = Request.MapPath(Request.ApplicationPath) + @"Reports\Versus\SopVsHipDetails.rdlc";
            var parameters = new List<ReportParameter>
                {
