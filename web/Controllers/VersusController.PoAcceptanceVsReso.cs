...
namespace Entt.Ers.Controllers
{
    ...
    {
        public ActionResult PoAcceptanceVsReso()
        {
        ...
        public ActionResult PoAcceptanceVsReso(PrefixReportViewModel model)
        {
            var reportViewer = ReportEngine.Create();
            ...
            if (ModelState.IsValid)
            {
                ...
                dataset = model.SelectedBranch == "All" ?
                    m_enttContext.GetPoAcceptanceVsResoReportDataSet(model.ReportDate, day) :
                    m_enttContext.GetPoAcceptanceVsResoBranchReportDataSet(model.ReportDate, day, model.SelectedBranch);
                reportViewer.LocalReport.ReportPath = Request.MapPath(Request.ApplicationPath) + @"Reports\Versus\PoAcceptanceVsReso.rdlc";
        ...
        public ActionResult PoAcceptanceVsResoDetails(string branchCode, double date, string day = "7")
        {
            ...
            var dataset = m_enttContext.GetPoAcceptanceVsResoDetailsReportDataSet(reportDate.Date, int.Parse(day), branchCode);
            reportViewer.LocalReport.ReportPath = Request.MapPath(Request.ApplicationPath) + @"Reports\Versus\PoAcceptanceVsResoDetails.rdlc";
            var parameters = new List<ReportParameter>
                {
