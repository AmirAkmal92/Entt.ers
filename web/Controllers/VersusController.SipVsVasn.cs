...
namespace Entt.Ers.Controllers
{
    public partial class VersusController
    {
        ...
        public ActionResult SipVsVasn(PrefixReportViewModel model)
        {
            var reportViewer = ReportEngine.Create();
            ...
            if (ModelState.IsValid)
            {
                ...
                dataset = model.SelectedBranch == "All" ?
                    m_enttContext.GetSipVsVasnReportDataSet(model.ReportDate, day) :
                    m_enttContext.GetSipVsVasnBranchReportDataSet(model.ReportDate, day, model.SelectedBranch);
                reportViewer.LocalReport.ReportPath = Request.MapPath(Request.ApplicationPath) + @"Reports\Versus\SipVsVasn.rdlc";
        ...
        public ActionResult SipVsVasnDetails(string branchCode, double date, string day = "7")
        {
            ...
            var dataset = m_enttContext.GetSipVsVasnDetailsReportDataSet(reportDate.Date, int.Parse(day), branchCode);
            reportViewer.LocalReport.ReportPath = Request.MapPath(Request.ApplicationPath) + @"Reports\Versus\SipVsVasnDetails.rdlc";
            var parameters = new List<ReportParameter>
                {
