...
namespace Entt.Ers.Controllers
{
    public partial class VersusController
    {
        ...
        public ActionResult SopVsSip(PrefixReportViewModel model)
        {
            var reportViewer = ReportEngine.Create();
            ...
            if (ModelState.IsValid)
            {
                ...
                dataset = model.SelectedBranch == "All" ?
                    m_enttContext.GetSopVsSipReportDataSet(model.ReportDate, day) :
                    m_enttContext.GetSopVsSipBranchReportDataSet(model.ReportDate, day, model.SelectedBranch);
                reportViewer.LocalReport.ReportPath = Request.MapPath(Request.ApplicationPath) + @"Reports\Versus\SopVsSip.rdlc";
        ...
        public ActionResult SopVsNoSipDetails(string branchCode, double date, string day = "7")
        {
            var user = m_dbContext.Users.Single(u => u.UserName == User.Identity.Name);
            if (null != user && !string.IsNullOrEmpty(user.BranchCode) && user.BranchCode != branchCode)
            {
            ...
            var dataset = m_enttContext.GetSopVsNoSipDetailsReportDataSet(reportDate.Date, int.Parse(day), branchCode);
            reportViewer.LocalReport.ReportPath = Request.MapPath(Request.ApplicationPath) + @"Reports\Versus\SopVsNoSipDetails.rdlc";
            var parameters = new List<ReportParameter>
                {
                    new ReportParameter("reportDate", reportDate.ToShortDateString()),
                    new ReportParameter("day", day),
